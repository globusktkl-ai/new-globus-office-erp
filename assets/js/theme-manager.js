...
class ThemeManager {
    ...
    async syncWithDatabase() {
        try {
            const { data, error } = await supabase.from('institute_settings').select('theme_color, allow_guest_login, guest_login_enabled').eq('id', 1).single();
            if (data && !error) {
                if(data.theme_color) localStorage.setItem('globus_theme_color', data.theme_color);
                
                const allowGuest = data.guest_login_enabled !== undefined ? data.guest_login_enabled : data.allow_guest_login;
                localStorage.setItem('globus_allow_guest', allowGuest);
